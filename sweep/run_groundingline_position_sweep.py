...
FREQUENCY_HZ = 10.0
DELTA_X_VALUES_M = [0.0, 1.0, 10.0, 100.0]
...
def main() -> None:
    ...
    results_dir = Path(__file__).resolve().with_name("results_glposition_10hz")
    ...
    backup_dir = Path(__file__).resolve().with_name("_output_files_backup_glposition")
    ...
    try:
        ...
        for delta_x_m in DELTA_X_VALUES_M:
            ...
            metadata_path = case_dir / "metadata.json"
            if metadata_path.exists():
                print(f"=== Grounding-line offset {delta_x_m:g} m already done; skipping ===", flush=True)
                continue

            # Positive delta_x means the grounding line moved to +x relative to
            # the fixed field array/source. We emulate that by shifting both the
            # source and receivers left by the same amount while keeping the
            # reflector fixed at x = 0.
            ...
            print(
                f"=== Grounding-line offset {delta_x_m:g} m: source/array shift {coordinate_shift_m:g} m ===",
                flush=True,
            )
